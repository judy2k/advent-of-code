...
class Monkey:
    ...
    def step(self, monkeys, mod):
        for item in self.items:
            new_value = self.operator(*[op(item) for op in self.operands]) % mod
...
def test_monkey_logic():
    ...
    for round in range(20):
        ...
        for m in ms:
            m.step(ms, mod)
    assert [m.inspected_count for m in ms] == [99, 97, 8, 103]


def test_sample():
    datafile = Path(__file__).parent.parent.joinpath("sample.txt").open()
    assert solve(datafile) == 2713310158


@pytest.mark.parametrize(
    ["p", "expected"],
    [("sample.txt", 4), ("input.txt", 8)],
)
...
def test_input():
    ...
    assert solve(datafile) == 14_508_081_294
